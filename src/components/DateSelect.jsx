                ...
                <div className='flex items-center gap-6 text-sm mt-5'>
                    ...
                    <span className='grid grid-cols-3 md:flex flex-wrap md:max-w-lg gap-4'>
                        {Object.keys(dateTime).map((date)=>(
                            // Object.keys() extracts just the keys (the dates) into an array
                            ...
                            ${selected === date ? 'bg-primary text-white':"border border-primary/70"}`}>
                                ...
                                {/* new Date("2024-01-15") */}
                                 {/* converts string into a JavaScript Date object */}
                                <span>{new Date(date).toLocaleDateString('en-US',{month:"short"})}</span>
                                 {/* formats the date showing only the short month name */}
                            </button>
                        ))}
                    </span>
